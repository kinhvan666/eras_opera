...
async function main() {
  ...
  try {
    ...
    if (envFile) {
      ...
      // Plan validation config for vc-kit RIPER-5 plan generation and validation workflows
    ...
    // MITIGATION: Issue #277 - Auto-compact can bypass approval gates
    ...
    if (source === 'compact') {
      ...
      console.log(`If you were waiting for user approval at any gate (for example a plan review or execute pause),`);
      console.log(`you MUST re-confirm with the user before proceeding. Do NOT assume approval was given.`);
      console.log(`Ask the user to confirm approval before continuing work.`);
    }
